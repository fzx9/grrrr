...
async def check(ctx):
    ...
    if "486603238427066369" in [role.id for role in ctx.message.author.roles]:#Must have endless role
        if wallsOn == True:#Checks wall checking is turned on            
            ...
            await bot.send_message(discord.Object(id="459437789856006155"), ctx.message.author.name)
...
async def setwalltime(ctx, time:int = None):#Sets the time to check walls
    if "426456861911416833" in [role.id for role in ctx.message.author.roles]:#Must be leader
        global wallsTime
        global count
...
#Bot token
bot.run("NDUzMjE1NzMyNjU0OTMxOTY5.DnBhUg.LV7ImZIjHcbWln_DLbEiAPJ0uFU")
